...
const create = async (planData) => {
    ...
    try {
        ...
        console.log(`${context} Vérification de l'existence de l'opérateur`, { operator_id: planData.operator_id });
        ...
        console.log(`${context} Plan inséré avec succès`, { insertId: result.insertId });
        ...
        console.log(`${context} Erreur lors de la création`, { error: error.message });
...
const findById = async (planId) => {
    ...
    try {
        ...
        const fieldsToDelete = [
            'operator_id_full', 'operator_name', 'operator_code',
            'operator_description', 'operator_active', 'operator_created_at',
            'operator_updated_at'
        ];
        fieldsToDelete.forEach(field => delete plan[field]);
        console.log(`${context} Champs temporaires supprimés`, { fieldsDeleted: fieldsToDelete.length });
        ...
        logger.error(`${context} Erreur lors de la recherche du plan avec relations`, {
            error: error.message,
            stack: error.stack,
            planId
        });
        throw new Error('Erreur lors de la récupération du plan avec ses relations');
    }
};
...
const findAll = async (includeInactive = false) => {
    ...
    console.log(`${context} Début de récupération des plans`, { includeInactive });

    try {
        logger.debug(`${context} Récupération des plans avec relations`, { includeInactive });
        ...
        console.log(`${context} Requête construite`, { queryLength: query.length, paramsCount: params.length });
        ...
        console.log(`${context} Tous les plans traités`, { totalPlans: plans.length });

        logger.info(`${context} Plans avec relations récupérés`, { count: plans.length });

        console.log(`${context} Récupération réussie`, { plansCount: plans.length });
        return plans;
        ...
        logger.error(`${context} Erreur lors de la récupération des plans avec relations`, {
            error: error.message,
            stack: error.stack
        });
        throw new Error('Erreur lors de la récupération de la liste des plans avec leurs relations');
    }
};
...
const update = async (planId, planData) => {
    ...
    try {
        ...
        if (planData.operator_id !== undefined) {
            console.log(`${context} Validation de l'opérateur_id`, { operator_id: planData.operator_id });
            // Vérifier que l'opérateur existe
            const [operators] = await db.execute(
                'SELECT id FROM operators WHERE id = ?',
                [planData.operator_id]
            ...
            if (operators.length === 0) {
                console.log(`${context} Opérateur non trouvé`, { operator_id: planData.operator_id });
                throw new Error('Opérateur non trouvé');
            }
        ...
        if (planData.validity_days !== undefined) {
            console.log(`${context} Mise à jour des jours de validité`, { validity_days: planData.validity_days });
            updates.push('validity_days = ?');
            params.push(planData.validity_days);
        }

        if (planData.active !== undefined) {
            console.log(`${context} Mise à jour du statut actif`, { active: planData.active });
            updates.push('active = ?');
        ...
        console.log(`${context} Requête UPDATE construite`, { queryLength: query.length });

        console.log(`${context} Exécution de la mise à jour`);
        const [result] = await db.execute(query, params);
        console.log(`${context} Mise à jour exécutée`, { affectedRows: result.affectedRows });
...
const deleteById = async (planId) => {
    ...
    try {
        ...
        console.log(`${context} Suppression exécutée`, { affectedRows: result.affectedRows });
        ...
        if (error.code === 'ER_ROW_IS_REFERENCED_2') {
            console.log(`${context} Contrainte de clé étrangère détectée lors de la suppression`);
...
const findByOperatorId = async (operatorId, includeInactive = false) => {
    ...
    try {
        ...
        logger.error(`${context} Erreur lors de la recherche des plans pour l'opérateur`, {
            error: error.message,
            stack: error.stack,
            operatorId
        });
        throw new Error('Erreur lors de la récupération des plans de l\'opérateur avec leurs relations');
    }
};
